...
export default function VerificationTasksModule() {
  ...
  // ── ORS route from user → nearest pending stop (foot-walking, resilient)
  // Strategy (mirrors ShiftRouteModule / NavigateToDumpsiteModule):
  ...
  useEffect(() => {
    ...
    if (!gpsPos || !nearestStop?.lat || !nearestStop?.lng) {
      // Clear the route layer when there's no valid target
    ...
    fetch(
      ...
      .catch(err => {
        ...
        console.warn('VerificationTasksModule: ORS unavailable, using straight-line fallback.', err.message)
  ...
  // This keeps the teal line visually anchored to the watcher's dot even without
    ...
    <>
      ...
      <div style={{ height: 'calc(100vh - 64px)', marginTop: 64, display: 'flex', flexDirection: 'column', overflow: 'hidden', position: 'relative' }}>
        ...
        {!loading && hasScheduleToday && nearestStop && gpsPos && haversineDistance(gpsPos.lat, gpsPos.lng, nearestStop.lat, nearestStop.lng) > 30 && (
          <div style={{ position: 'absolute', top: 120, left: 14, right: 14, zIndex: 10, background: 'rgba(255,255,255,0.97)', borderRadius: 16, overflow: 'hidden', display: 'flex', alignItems: 'stretch', boxShadow: '0 6px 28px rgba(0,0,0,.18)', backdropFilter: 'blur(6px)', transition: 'top .3s ease' }}>
